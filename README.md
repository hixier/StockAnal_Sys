...
- **实时数据获取**：通过Binance API获取价格数据，CoinGecko API获取基本面数据
- **技术分析**：移动平均线、价格趋势、成交量分析
- **基本面分析**：市值排名、价格表现、流动性评估
- **智能评分**：技术面和基本面综合评分，提供投资建议
- **热门币种**：展示市值排名前N的加密货币
- **市场概览**：实时显示加密货币市场状态
...
├── crypto_analyzer.py       # 加密货币分析引擎
├── crypto_api_endpoints.py  # 加密货币API端点
...
- 加密货币分析API：`/api/crypto/analyze`
- 加密货币列表API：`/api/crypto/top`
- 加密货币搜索API：`/api/crypto/search`

## 📋 版本历史

### v2.2.0 (当前版本)
- 新增加密货币分析功能，支持BTC、ETH等主流币种分析
- 集成Binance和CoinGecko API，获取实时价格和基本面数据
- 提供加密货币技术面和基本面评分系统
- 新增加密货币分析页面和API接口
